...
module.exports.set = function( program, conf ) {
    ...
    ServerPilotCLI.prototype.servers = function( actions ) {
         ...
         * @return {void}
         */
        function deleteServer(spcli, serverId) {
            console.log();
            console.log('Attempting to delete the server ' + serverId + '...');
            console.log();

            spcli.sp.deleteServer(serverId, function(err, data) {
                if (err) {
                    console.log(chalk.red.bold('Error: ' + err.message));
                } else {
                    console.log(chalk.green.bold('Successfully deleted server ' + serverId));
                }
            });
        }
                    ...
                    if ( actions.length > 1 ) {
                        ...
                        this.sp.getServers(function(err, data) {
                            ...
                            inquirer.prompt({
                                ...
                                var serverIdToDelete = result.server;

                                // Confirm deletion
                                inquirer.prompt({
                                    type: 'confirm',
                                    name: 'deleteServer',
                                    message: 'Are you sure you want to delete server ' + result.server + '?'
                                }, function(result) {
                                    if ( result.deleteServer ) {
                                        deleteServer(this, serverIdToDelete);
                                    }
                                }.bind(this));
                            }.bind(this));
                        }.bind(this));
